...
async function handleUpdateData(dataType: string, sex: number, values: string) {
    ...
    if (dataType === 'appearanceData') {
        ...
        for (let i = 0; i < data.appearanceValue.length; i++) {
            if (i === 1 || i === 2) continue;
            const appearance = data.appearanceValue[i];
            if (appearance === undefined) continue;
            native.setPedHeadOverlay(player, i, appearance.index - 1, appearance.opacity / 100);
            if (i === 5) native.setPedHeadOverlayTint(player, 5, 2, appearance.color, 0);
            if (i === 8) native.setPedHeadOverlayTint(player, 8, 2, appearance.color, 0);
        }
        appearanceData = data;
        return;
    }
...
function updateClothes(sex: number, values: string) {
    ...
    if (data.hats[1] == -1) {
        ...
        const props = sex === 0 ? PROPS_MALE_HATS : PROPS_FEMALE_HATS;
        const key = data.hats[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        });

        if (colors.length < 1) colors = [0];
        native.setPedPropIndex(player, 0, data.hats[1], colors[data.hats[2]] as number, true, 0);
        equipment.hats = [data.hats[1], colors[data.hats[2]] as number, 'Default'];
    ...
    if (data.glasses[1] == -1) {
        ...
        const props = sex === 0 ? PROPS_MALE_GLASSES : PROPS_FEMALE_GLASSES;
        const key = data.glasses[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        ...
        if (colors.length < 1) colors = [0];
        native.setPedPropIndex(player, 1, data.glasses[1], colors[data.glasses[2]] as number, true, 0);
        equipment.glasses = [data.glasses[1], colors[data.glasses[2]] as number, 'Default'];
    ...
    if (data.watches[1] == -1) {
        ...
        let colors: Array<number> = [];
        const props = sex === 0 ? PROPS_MALE_WATCHES : PROPS_FEMALE_WATCHES;
        const key = data.watches[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        ...
        if (colors.length < 1) colors = [0];
        native.setPedPropIndex(player, 6, data.watches[1], colors[data.watches[2]] as number, true, 0);
        equipment.watches = [data.watches[1], colors[data.watches[2]] as number, 'Default'];
    ...
    if (data.ears[1] == -1) {
        ...
        let colors: Array<number> = [];
        const props = sex === 0 ? PROPS_MALE_EARS : PROPS_FEMALE_EARS;
        const key = data.ears[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        ...
        if (colors.length < 1) colors = [0];
        native.setPedPropIndex(player, 2, data.ears[1], colors[data.ears[2]] as number, true, 0);
        equipment.ears = [data.ears[1], colors[data.ears[2]] as number, 'Default'];
    ...
    if (data.bracelets[1] == -1) {
        ...
        let colors: Array<number> = [];
        const props = sex === 0 ? PROPS_MALE_BRACELETS : PROPS_FEMALE_BRACELETS;
        const key = data.bracelets[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        ...
        if (colors.length < 1) colors = [0];
        native.setPedPropIndex(player, 7, data.bracelets[1], colors[data.bracelets[2]] as number, true, 0);
        equipment.bracelets = [data.bracelets[1], colors[data.bracelets[2]] as number, 'Default'];
    ...
    if (data.tops[1] == -1) {
        ...
        const torsos = sex === 0 ? maleTorsoCombos : femaleTorsoCombos;
        torsos[data.tops[1]]?.map((val) => {
    ...
    if (data.pants[1] == -1) {
        ...
        let colors: Array<number> = [];
        const props = sex === 0 ? MALE_LEGS : FEMALE_LEGS;
        const key = data.pants[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        ...
        if (colors.length < 1) colors = [0];
        native.setPedComponentVariation(player, 4, data.pants[1], colors[data.pants[2]] as number, 2);
        equipment.pants = [data.pants[1], colors[data.pants[2]] as number, 'Default'];
    ...
    if (data.shoes[1] == -1) {
        ...
        let colors: Array<number> = [];
        const props = sex === 0 ? MALE_SHOES : FEMALE_SHOES;
        const key = data.shoes[1].toString();
        Object.keys(props[key as keyof typeof props]).forEach((v, index) => {
            if (props[key as keyof typeof props][v as keyof (typeof props)[keyof typeof props]].GXT !== 'NO_LABEL') {
                colors.push(index);
            }
        ...
        if (colors.length < 1) colors = [0];
        native.setPedComponentVariation(player, 6, data.shoes[1], colors[data.shoes[2]] as number, 2);
        equipment.shoes = [data.shoes[1], colors[data.shoes[2]] as number, 'Default'];
