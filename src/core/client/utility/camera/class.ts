...
export default class Camera {
    ...
    set fov(value: number) {
        this._fov = value;
        native.setCamFov(this.scriptID[0], this._fov);
        this.render();
    }
    ...
    setPosition(position: alt.Vector3) {
        this._position = position;
        native.setCamCoord(this.scriptID[0], this._position.x, this._position.y, this._position.z);
        this.render();
    }
    ...
    setRotation(rotation: alt.Vector3) {
        this._rotation = rotation;
        native.setCamRot(this.scriptID[0], this._rotation.x, this._rotation.y, this._rotation.z, 0);
     ...
     */
    render() {
        native.setCamActive(this.scriptID[0], true);
    ...
    destroy() {
        this.scriptID.forEach((val, i) => {
            native.destroyCam(this.scriptID[i], false);
        });
    ...
    pointAtBone(entity: number, bone: number, xOffset: number, yOffset: number, zOffset: number) {
        native.pointCamAtPedBone(this.scriptID[0], entity, bone, xOffset, yOffset, zOffset, false);
        this.render();
    }
    ...
    pointAtCoord(position: alt.Vector3) {
        native.pointCamAtCoord(this.scriptID[0], position.x, position.y, position.z);
        this.render();
    }
    ...
    interpolate(pos: alt.Vector3, lookAt: alt.Vector3, duration: number) {
        ...
        native.pointCamAtCoord(this.scriptID[0], lookAt.x, lookAt.y, lookAt.z);
        native.setCamActiveWithInterp(this.scriptID[0], this.scriptID[1], duration, 1, 1);
        native.renderScriptCams(true, false, 0, true, false, 0);
    }
}
