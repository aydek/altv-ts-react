...
const CharCreator = () => {
    ...
    useEffect(() => {
        if ('alt' in window) alt.emit('Webview:Character:Creator:Update:Data', 'parentData', sex, JSON.stringify(parentData));
    }, [parentData]);

    useEffect(() => {
        if ('alt' in window) alt.emit('Webview:Character:Creator:Update:Data', 'hairData', sex, JSON.stringify(hairData));
    }, [hairData]);

    useEffect(() => {
        if ('alt' in window) alt.emit('Webview:Character:Creator:Update:Data', 'eyesData', sex, JSON.stringify(eyesData));
    }, [eyesData]);

    useEffect(() => {
        if ('alt' in window) alt.emit('Webview:Character:Creator:Update:Data', 'featuresData', sex, JSON.stringify(featuresData));
    }, [featuresData]);

    useEffect(() => {
        if ('alt' in window) alt.emit('Webview:Character:Creator:Update:Data', 'appearanceData', sex, JSON.stringify(appearanceData));
    }, [appearanceData]);

    useEffect(() => {
        if ('alt' in window) {
            if (emitClothes.current) alt.emit('Webview:Character:Creator:Update:Data', 'clothesData', sex, JSON.stringify(clothesData));
    ...
    return (
        <div className="select-none">
            ...
            <div className="absolute right-0 top-0 text-whitesmoke w-[40%] h-screen bg-gradient-to-l from-black flex items-center justify-end">
